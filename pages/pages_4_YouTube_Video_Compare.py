...
if st.button("Process URLs"):
    ...
    with col11:
        ...
        if len(words) < 11:
            st.warning("Error: At least 11 unique words are required for top tokens bar chart.")
        else:
            # Create a figure and bar chart
            with _lock:
                plt.figure(1, figsize=(8, 4))  # Set figure number
                ...
                plt.xlabel("Tokens")
                plt.ylabel("Count")
                plt.yscale('linear')  # Force y-axis to display integers
        ...
        for idx, transcript in enumerate(user_transcripts, start=1):
            ...
            st.write(summary_str)  # Display the summary
    ...
    with col22:
        ...
        if len(words1) < 11:
            ...
            with _lock:
                ...
                # Display the plot in Streamlit
                st.pyplot(plt.figure(10), use_container_width=True)
        ##########
        ...
        with _lock:
            ...
            st.pyplot(plt.figure(11), use_container_width=True)
        ...
        with _lock:
            if pos_tokens1:
                ...
                st.pyplot(plt.figure(12), use_container_width=True)
            else:
                plt.figure(12)
                plt.text(0.5, 0.5, "No positive words found", horizontalalignment='center', verticalalignment='center', fontsize=12, color='red')
                plt.axis("off")
        ...
        with _lock:
            if neg_tokens1:
                ...
                st.pyplot(plt.figure(13), use_container_width=True)
            else:
                plt.figure(13)
                plt.text(0.5, 0.5, "No negative words found", horizontalalignment='center', verticalalignment='center', fontsize=12, color='red')
                plt.axis("off")
        ...
        with _lock:
            ...
            st.pyplot(plt.figure(14), use_container_width=True)
        ##############################################################################################
        ...
        with _lock:
            ...
            st.pyplot(plt.figure(15), use_container_width=True)
        ...
        with _lock:
            ...
            plt.show()
            st.pyplot(plt.figure(16), use_container_width=True)
        ...
        for idx, transcript1 in enumerate(user_transcripts1, start=1):
            ...
            # Generate and display the summary
            parser1 = PlaintextParser.from_string(transcript, Tokenizer("english"))
            summarizer1 = LsaSummarizer()
            summary1 = summarizer1(parser1.document, sentences_count=7)
            summary_str1 = ' '.join([str(sentence) for sentence in summary1])
            st.subheader(f"Summary for Transcript {idx}:")
            st.write(summary_str1)  # Display the summary
    ...
    text01 = text + text1
    parser01 = PlaintextParser.from_string(text01, Tokenizer("english"))
    summarizer01 = LsaSummarizer()
    summary01 = summarizer01(parser01.document, sentences_count=7)
    summary_str01 = ' '.join([str(sentence) for sentence in summary01])
    st.subheader(f"Common Summary")
    st.write(summary_str01)  # Display the summary
