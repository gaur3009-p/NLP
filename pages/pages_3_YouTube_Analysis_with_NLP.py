...
if process_button:
    ...
    for idx, transcript in enumerate(user_transcripts, start=1):
        ...
        # result00 = model01(body=transcript)
        # full00 = ''.join(result00)
        # print(full00)
    ...
    commom_parser = PlaintextParser.from_string(text, Tokenizer("english"))
    summarizer1 = LsaSummarizer()
    commom_summary = summarizer1(commom_parser.document, sentences_count=10)
    commom_summary_str = ' '.join([str(sentence) for sentence in commom_summary])
    

    st.subheader("Common Keywords:")
    st.write(common_keywords)
    st.subheader("Common Summary:")
    st.write(commom_summary_str)  # Display the summary
    st.balloons()
